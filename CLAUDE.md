...
On the first interaction of a new Claude Code session (NOT on /clear), check whether the `supabase/postgres` image tag used in this repo matches the one in the `gmautner/locaweb-cloud-provision` GitHub repo. That repo has a GitHub Actions workflow that auto-bumps the tag weekly from Docker Hub.

Tell the user you are running this check. Procedure:
1. Fetch `config/deploy.yml` from `gmautner/locaweb-cloud-provision` using `gh api` and search for the current `supabase/postgres` version tag.
