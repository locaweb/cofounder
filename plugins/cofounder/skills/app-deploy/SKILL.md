...
- **Volume pattern: `/data/{subdir}`** -- always mount subdirectories of `/data/`, never `/data/` root directly (see [references/env-vars.md -- Web Disk Storage Path](references/env-vars.md#web-disk-storage-path) for usage). The ext4 filesystem creates `lost+found` at the mount root, which breaks Docker images that expect a clean directory on first boot (PostgreSQL `initdb`, Redis `appendonly.aof`, etc.).
...
- **Accessories** -- image, host (ERB), port, cmd, env, volumes
