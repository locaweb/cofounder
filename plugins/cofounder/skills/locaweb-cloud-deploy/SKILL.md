...
- **`.kamal/secrets.<destination>`** (e.g., `.kamal/secrets.preview`, `.kamal/secrets.production`) -- one file per destination, mapping GitHub Secret names to Kamal secret env vars. Each file is a self-contained manifest of what that environment needs. Registry is handled by the workflow.
...
| **Agent** | Application config | `deploy.yml`, `.kamal/secrets.<dest>`, env vars, accessories, volumes, worker commands |
...
#### 7c: Secrets files (`.kamal/secrets.<destination>`)

Since all deploys use `-d <destination>`, Kamal looks for `.kamal/secrets-common` and `.kamal/secrets.<destination>` — **not** `.kamal/secrets`. We use **only per-destination files** so each file is a self-contained, verifiable manifest of what that environment needs.

The agent writes one secrets file per destination:
...
Each line maps a Kamal secret name to a shell env var. Kamal reads these values from the runner environment, where GitHub Actions automatically injects repository secrets. The registry credential (`KAMAL_REGISTRY_PASSWORD`) is handled by the workflow -- do not include it here.
