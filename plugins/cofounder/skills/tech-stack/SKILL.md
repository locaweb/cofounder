...
### Entering secrets in `.env`

**Never accept secret values through the chat.** Secrets must be entered by the user directly in their text editor or auto-generated by the agent — never pasted into the conversation.

Secrets fall into two categories:

#### Secrets the agent generates directly

Secrets that are just random strings (JWT signing keys, session secrets, HMAC keys, internal tokens, etc.) should be generated by the agent and written straight into `.env` — never ask the user to come up with random values. Use:

```bash
mise x -- python -c "import secrets; print(secrets.token_urlsafe(32))"
```

#### Secrets only the user knows

External credentials (OAuth client IDs/secrets, SMTP passwords, third-party API keys, etc.) can only come from the user. Follow this procedure:

1. **Write `.env` with placeholders.** Create or update the file with descriptive placeholder values for every secret the application needs:
   ...
   Use the `REPLACE_WITH_` prefix for user-provided secrets so placeholders are easy to detect programmatically.

2. **Open the file in the user's editor.** Verify the `.env` file exists at the expected absolute path, then spawn the platform-appropriate editor:
   ...
   Always use the full absolute path to the `.env` file.

3. **Ask the user to save and confirm.** Tell the user which placeholder values need to be replaced and walk them through how to obtain each credential using the relevant reference doc (e.g., `references/google-auth.md`, `references/smtp-gateway.md`). Ask them to save the file and confirm when done.

4. **Validate without exposing.** Read the `.env` file and check that the specific placeholders the user was asked to fill in no longer contain the `REPLACE_WITH_` prefix. Report success or list which placeholders remain — **never output the actual secret values in the chat**.
