---
name: cofounder-frontend-design
...
- **Spacing & Rhythm**: Maintain consistent spacing between elements as a non-negotiable foundation. Use Tailwind's spacing scale (`p-`, `m-`, `gap-`, `space-y-`, `space-x-`) systematically. Every container needs appropriate padding. Every list of items needs consistent gaps. Every section needs breathing room from its neighbors. Creative layouts are built ON TOP of solid spacing fundamentals — never sacrifice padding and margins for aesthetic experimentation. When customizing or overriding component styles, explicitly preserve or intentionally adjust spacing rather than letting it collapse to zero.
